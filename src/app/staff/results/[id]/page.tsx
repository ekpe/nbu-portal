      ...
      <main className="space-y-6 p-6">
        <div>
          ...
          <p className="mt-2 text-gray-600">
            {sheet.offering.course.courseCode} - {sheet.offering.course.title} | {sheet.offering.session.name} | {sheet.offering.semester.name}
          </p>
        ...
        <div className="rounded-2xl border bg-white p-6">
          ...
          <p><strong>Course Title:</strong> {sheet.offering.course.title}</p>
              ...
              {sheet.entries.map((entry) => (
                <tr key={entry.id} className="border-t align-top">
                  ...
                  <td className="px-3 py-2">
                    {entry.studentProfile.user.firstName} {entry.studentProfile.user.lastName}
                  </td>
                  <td className="px-3 py-2">
                    <form action={saveResultEntryAction} className="flex flex-wrap gap-2">
                      <input type="hidden" name="entryId" value={entry.id} />
                      <input name="caScore" type="number" step="0.5" min="0" max="30" defaultValue={entry.caScore ?? ""} className="w-20 rounded border px-2 py-1" />
                  </td>
                  <td className="px-3 py-2">
                      <input name="examScore" type="number" step="0.5" min="0" max="70" defaultValue={entry.examScore ?? ""} className="w-20 rounded border px-2 py-1" />
                  </td>
                  <td className="px-3 py-2">{entry.totalScore ?? "-"}</td>
                  <td className="px-3 py-2">{entry.letterGrade ?? "-"}</td>
                  <td className="px-3 py-2"><input name="q1Score" type="number" step="0.5" defaultValue={entry.q1Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2"><input name="q2Score" type="number" step="0.5" defaultValue={entry.q2Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2"><input name="q3Score" type="number" step="0.5" defaultValue={entry.q3Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2"><input name="q4Score" type="number" step="0.5" defaultValue={entry.q4Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2"><input name="q5Score" type="number" step="0.5" defaultValue={entry.q5Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2"><input name="q6Score" type="number" step="0.5" defaultValue={entry.q6Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2"><input name="q7Score" type="number" step="0.5" defaultValue={entry.q7Score ?? ""} className="w-16 rounded border px-2 py-1" /></td>
                  <td className="px-3 py-2">
                      {sheet.status === "DRAFT" ? (
                        <button className="rounded bg-gray-900 px-3 py-1 text-white">Save</button>
                      ) : (
                        "-"
                      )}
                    </form>
